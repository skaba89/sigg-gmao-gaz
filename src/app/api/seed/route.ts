...
import { hashPassword, withAuth, type TokenPayload } from '@/lib/auth-utils';

// Seed endpoint - PROTECTED: Requires SUPER_ADMIN role + SEED_KEY header
export const GET = withAuth(
  async (request: Request, _context, user: TokenPayload) => {
    try {
      // Double protection: require SEED_KEY header
      const seedKey = request.headers.get('x-seed-key');
      if (seedKey !== process.env.SEED_KEY) {
        return Response.json({ error: 'Clé de seed invalide' }, { status: 403 });
      }
      ...
      console.log('🌱 Seeding SIGG GMAO database... (by:', user.email, ')');
      ...
      for (let i = 0; i < woData.length; i++) {
        ...
        const wo = await db.workOrder.create({
          data: {
            ...
            actualStartDate: (d as any).actualStartDate ? new Date((d as any).actualStartDate) : undefined,
            actualEndDate: (d as any).actualEndDate ? new Date((d as any).actualEndDate) : undefined,
            estimatedHours: d.estimatedHours,
            actualHours: (d as any).actualHours,
          },
        });
        workOrders.push(wo);
      }
